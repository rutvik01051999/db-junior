                ...
                <div class="card-body">
                    <form id="videoForm" action="{{ route('admin.videos.update', $video->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <label for="video">Change Video <small class="text-muted">(Leave empty to keep current video, Max: 100MB, Supported formats: MP4, MOV, AVI, WMV)</small></label>
                            <div class="custom-file">
                                <input type="file" class="custom-file-input @error('video') is-invalid @enderror" id="video" name="video" accept="video/*">
                                <label class="custom-file-label" for="video">Choose new video file (optional)</label>
                                @error('video')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
